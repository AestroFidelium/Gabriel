...
class MyClient(discord.Client):
    ...
    async def CheckGroup(self,_Group : Group):
        Group = _Group
        Replaces = ['/',"|","Ɛ",":"]
        MainName = Group.Name
        for replace in Replaces:
            MainName = MainName.replace(replace,"")
        
        try:
            with codecs.open(f"./Resurses/loli/OldLolies/{MainName}.txt","r",encoding='utf-8') as file:
                OldLolies = StrToDict(str(file.readline()))
                OldLolies = OldLolies['list']
        except: 
            with codecs.open(f"./Resurses/loli/OldLolies/{MainName}.txt","w",encoding='utf-8') as file:
                file.write(str({'list':[]}))
            OldLolies = []
        print(f"[{Group.Name}] готова")
        while True:
            be = False
            NewLolies = 0
            Posts = list()
            try:
                ...
                MainCity = requests.get(Group.URL,headers = headers)
                ...
                for post in PostsList.find_all("div",{"class":"_post"}):
                    try:
                        post = post.find('div',{"class":"wall_text"})
                        Tags = list()
                        try:
                        ...
                        if str(_Post) not in OldLolies:
                            be = True
                            Posts.append(_Post)
                            OldLolies.append(str(_Post))
                            NewLolies += 1
                    except: pass
                if be == True: print(f"{Group.Name} новых постов ({NewLolies})")
                with codecs.open(f"./Resurses/loli/OldLolies/{MainName}.txt","w",encoding='utf-8') as file:
                    file.write(str({"list":OldLolies}))
            except BaseException as Error: print(f"{Group.Name} {Error}")
            if be == True:
                print(f"{Group.Name} отправка сообщений")
                for post in Posts:
                            ...
                            for _embed in Embeds:
                                ...
                                if len(Embeds) >= 10:
                                    ...
                                    if Count == 10:
                                        ...
                                        Count = 0
                                        for __embed in _Embeds:
                                            Embeds.remove(__embed)
                                else:
                                    await self.webhook.send(
                                            embeds = Embeds,
                                            content = post.Content,
