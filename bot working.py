...
def ReadFile(_User_):
    ...
    StatsRd = open("StatsPlayer_" + _User_ + ".txt","r") #Открывае поток файлов, на чтение
    IntCurExp = StatsRd.readline() #Читаем 1 строчку, где храниться опыт
    IntCurLvl = StatsRd.readline() #Читаем 2 строчку, где храниться уровень
    IntMaxHealth = StatsRd.readline() #Читаем 2 строчку, где храниться уровень
    IntCurHealth = StatsRd.readline() #Читаем 2 строчку, где храниться уровень
    IntMaxDamage = StatsRd.readline()
    Description = StatsRd.readline()
    StatsRd.close()
    return IntCurExp , IntCurLvl , IntMaxHealth , IntCurHealth , IntMaxDamage , Description

def WriteableFile(_User_,_Dates_):
    StatsWr = open("StatsPlayer_" + _User_ + ".txt","w") #Открывае поток файлов, на чтение
    for target_list in range(len(_Dates_)):
        StatsWr.writelines(_Dates_[target_list])
    StatsWr.close()
...
class MyClient(discord.Client):
    ...
    # don't respond to ourselves
        ...
        try:
            BazeData = ReadFile(UserName_)
            #lst = list
            IntCurExp = BazeData[0]
            IntCurLvl = BazeData[1]
            IntMaxHealth = BazeData[2]
            IntCurHealth = BazeData[3]
            IntMaxDamage = BazeData[4]
            Description = BazeData[5]
            fs = tuple
            fs += "s"
            print(fs)
        ...
        if (CurCommand == "ATTACK") or (CurCommand == "A") or (CurCommand == "А") or (CurCommand == "АТАКА") or (CurCommand == "АТАКОВАТЬ") or (CurCommand == "АТАКУЮ"):
            ...
            try:
                if CurCommandPlayer == UserName_:
                    await message.channel.send("Нельзя выбрать в качестве цели самого себя")
                    return
        ...
        if (CurCommand == 'PROFILE') or (CurCommand == 'P') or (CurCommand == 'П') or (CurCommand == 'ПРОФИЛЬ'):
            ...
            try:
                if CurCommandPlayer != "Сайтама":
                    ...
                    Emb = discord.Embed( title = CurCommandPlayer)
                    Emb.add_field(name = 'Могущество : ',value = "Запредельное")
                    Emb.add_field(name = 'Опыт : ',value = "∞",inline = False)
                    Emb.add_field(name = 'Здоровье : ',value = "∞"+ " / " + "∞",inline = False)
                    Emb.add_field(name = 'Ранг : ',value = "One-Punch Man",inline = False)
                    await message.channel.send(embed = Emb)
            except FileNotFoundError:
                if CurCommandPlayer == "":
                    pass
